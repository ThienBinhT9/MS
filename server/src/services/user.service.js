  ...
  async getDetailUser(client_id, user_id) {
    try {
      ...
      if (client_id.toString() === user_id)
        return { code: 200, metadata: passFeild };
  ...
  async ResetPassword(token, otp, newPassword, oldPassword, userId){
      try {
        if(!token || !otp || !newPassword)
          return {code:400, message:"Token, OTP, and new password are required."}
        ...
        if(!userAfterUpdate) return { code:400, message:"Đổi mật khẩu không thành công. Xin thử lại lần nữa!" }
