...
function BarUser(props: Props) {
  const { className, classNamePopover, options } = props;
  return (
    <div className={`wrapper-bar-user ${className}`}>
      <Popover
        style={{ width: 200 }}
      ...
      {options && <EllipsisOutlined className="bar-user-right" />}
    </div>
  );
}
